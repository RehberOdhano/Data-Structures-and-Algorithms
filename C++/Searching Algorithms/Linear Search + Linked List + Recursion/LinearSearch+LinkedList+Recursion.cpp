...
int LinearSearch(int count, int elementToBeSearched);
...
int main()
{
	...
	int elementReturned = LinearSearch(0, input);
...
{	
	...
	else if(head -> data = elementToBeSearched)
	{
		return count;
	}
	else
	{
		head = head -> next;
		LinearSearch(count += 1, elementToBeSearched);
	}
}
